...
- **根因**：执行付款计划按 PO 现存付款记录数量加一生成单号；历史编号不连续时可能复用已有单号，触发 `uq_payment_records_payment_number` 并返回 500。
- **编号分配**：合同与 PO 的付款计划执行按完整单号前缀下的最大数字后缀加一分配新号，不重写已有付款记录；正确处理超过三位的编号及前缀中的 SQL 通配字符。
- **并发保护**：事务内锁定并刷新 PO 和目标期次，串行处理同一 PO 的计划执行及已付金额累加；等待锁后重新检查付款状态，避免同一期次重复入账。
- **锁顺序**：执行按“计划项 → PO”加锁，与付款删除流程一致；两项真实数据库回归覆盖删除与执行交错时的死锁及金额一致性。
- **验证**：新增 19 项 HTTP、编号和真实 PostgreSQL 并发回归。后端全量 693 passed，覆盖率 71.96%；前端 type-check/build 与后端 Ruff 检查通过。
- **端到端**：最终修复在隔离环境由采购经理通过浏览器执行第二期返回 200，生成新单号 003，保留旧单号 002；付款金额、PO 已付汇总及剩余期次状态正确。

---

## [v1.51.1] — 2026-08-28

### 修复（付款计划在已有付款后重新安排时报 500）

- **根因**：整体保存付款计划时，后端只删除 `planned` 期次以保护已付款历史，却继续使用前端传入的 `installment_no = 1..n` 插入新行。已有已付款第 1 期时，新第 1 期触发 `uq_schedule_contract_installment` 唯一约束，未转换的数据库异常返回为 500。
- **安全重编号**：`replace_schedule()` 现先删除直属父对象下的计划中期次，再读取保留记录的最大期号；新提交行由服务端从该期号后顺序编号，不再信任客户端期号。
- **历史不可变**：`paid`、`partially_paid`、`due`、`cancelled` 期次的 ID、期号、金额、实际付款、日期、付款/发票关联和审计含义均保持不变。
- **覆盖范围**：合同与直接 PO 付款计划共用同一修复逻辑。
- **回归测试**：新增合同已付款、合同部分付款、直接 PO 已付款三组替换测试，验证旧的计划中行被替换、新行从保留历史期号后开始编号；同时更新一条旧测试以反映服务端期号为权威。
