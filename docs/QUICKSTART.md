...
4. 右上角有**主题切换**（🌙 / ☀️ / 💻 跟随系统），支持浅色+暗色两种模式
5. 左侧菜单选 **采购申请** → 点击 **新建**
...
- **Admin 控制台**（7 个标签页）：系统信息 / 系统参数 / LLM 模型 / AI 路由 / 用户 / AI 日志 / 审计日志
- **系统参数可编辑**：14 项业务阈值（审批金额、JWT 有效期、SKU 窗口、合同提醒、上传限制 …）从 Admin → 系统参数标签页编辑，无需改代码
...
- 飞书集成骨架已就绪但未接通真实 App ID/Secret（v0.6 路线图）
- 批量导入（Excel → 供应商 / 物料 / 报价）在 v0.6+ 规划
