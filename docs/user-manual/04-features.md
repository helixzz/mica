...
| 数据 | 作用 | 谁写入 |
|:---|:---|:---|
| **报价记录（Price Record）** | 某时某供某物的具体价格 | 采购员手工录入 / 实际成交的 PO 自动回填（v0.5+） |
...
- **当前 v0.4.2**：PostgreSQL `ILIKE` 模糊匹配。适合 < 1000 份合同的场景。
- **v0.6 升级计划**：PG `tsvector` + `zhparser` 中文分词 + GIN 索引，查询速度显著提升，支持精确短语 / 布尔查询。

### 合同到期提醒

- API：`GET /contracts-expiring?within_days=30`
- UI：合同列表页自动显示 30 天内到期的 active 状态合同
- 仪表盘集成（规划中 v0.5）：给相关角色自动推送
...
- **v0.5**
  - 飞书集成：消息通知 + 付款审批联动（3 个权限码）
  - Cerbos 引擎接入 + 行级 / 属性级细粒度权限
  - PO 成交价自动回填到 SKU 价格库
- **v0.6**
  - PostgreSQL zhparser 中文全文检索（升级合同 OCR 搜索）
  - 云服务商账单自动生成月度 PO
- **v0.7**
  - 飞书付款申请端到端自动化
- **按需**
