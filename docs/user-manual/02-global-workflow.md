...
完整版流程图请看 [`_diagrams.md`](./_diagrams.md)，也可以在本页直接查看简化版：
...
> 完整版见 [`_diagrams.md`](./_diagrams.md)。
...
> **v0.5 起支持多级审批**：通过 `approval_rules` 按 `biz_type + 金额区间` 配置串签阶段（种子默认：< ¥100,000 单级 dept_manager；≥ ¥100,000 两级 dept_manager → procurement_mgr）。管理员可在 Admin 控制台编辑规则，亦可创建**审批代理人**（出差时临时委托他人）。
...
可以直接当面/消息提醒。**v0.5 起系统会自动推送站内通知**（Header 铃铛 + 通知中心页）到审批人；**飞书推送**在 v0.6 路线图中。
...
> 多供应商 PR 一键拆分、批量转 PO 在**规划中 v0.7+**（当前通过手动拆 PR 解决）。
...
> **v0.4.2 起**已支持发票影像 AI 提取（6 级分层：XML 原件 → PDF 结构化 → OFD → 图片 OCR → LLM Vision 兜底），Carol 在上传后可直接复核 AI 提取结果。多发票批量上传在 v0.7+ 规划。
...
> 与飞书审批联动在 **v0.6** 路线图中（等 App ID+Secret）。付款前多级审批可在 `approval_rules` 中为 `biz_type = payment` 配置新规则。
