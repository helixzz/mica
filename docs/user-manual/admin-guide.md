...
当用户通过 SSO 登录时，系统按照**两级查找**将 IdP 身份映射到本地用户：

**第一级 — `sso_external_id` 精确匹配**

系统首先用 IdP 返回的 NameID（通常是邮箱地址）在 `users.sso_external_id` 字段上做精确查找。如果命中，直接更新该用户的显示名、邮箱等属性并完成登录。

**第二级 — `email` 回退匹配**

如果 `sso_external_id` 没有命中，系统会用 IdP 传回的邮箱地址在 `users.email` 字段上查找。此时有三种情况：

| 场景 | 系统行为 |
|:---|:---|
| 邮箱也未命中 | 自动创建新用户（JIT），`auth_provider` 设为 `saml` |
| 邮箱命中，且该用户尚未绑定其他 SSO 身份 | **自动关联**：将 SSO 身份写入该本地用户，`auth_provider` 改为 `saml` |
| 邮箱命中，但该用户已绑定了不同的 SSO 身份 | **拒绝登录**（HTTP 409），防止身份冲突 |

**实际含义**：

- 如果管理员先在系统中用本地账号创建了 `alice@company.com`，之后 Alice 通过 SSO 登录（IdP 传回同样的邮箱），系统会自动把她的本地账号关联到 SSO，无需重新创建账号，历史数据也不会丢失。
- 关联后，该用户仍可通过"使用本地账号登录"入口用原密码登录（如果之前设过密码）。
- `username` 字段**不参与**身份匹配。JIT 自动创建用户时，系统会从邮箱 `@` 前面的部分生成用户名（如 `alice`，重复时自动加后缀 `alice_2`）。

!!! tip "排查 SSO 登录问题"
    如果用户反馈 SSO 登录报 409 错误，说明该邮箱已被关联到另一个 SSO 身份。管理员可在 **用户管理** 中检查该用户的 `auth_provider` 和 SSO 绑定状态。
