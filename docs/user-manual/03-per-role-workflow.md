...
- **合同 Contracts** — 我负责的 PO 对应合同（只读查看）
...
8. **等待审批结果**
   ...
   - 你什么都不用做，关掉去忙别的事。系统有通知时会提醒你（v0.5 会接入飞书推送）。
   - 过一会儿回来刷新详情页，状态可能变为：
...
3. **我休假时待办怎么办？审批会堵吗？**
   当前 v0.4 版本**尚不支持审批委托和代办**。如需临时调整，请联系管理员——管理员作为兜底审批人可代你处置（见管理员章节）。这是已知限制，审批委托 / 多级审批规划在 v0.5+ 的路线图中。
...
2. **场景 A：登记一张发票**
   ...
   - 保存后发票状态为 `matched`（所有产品行都有对应 PO item 且不超额）或 `pending_match`（有超额 / 无 PO 对应 / 杂费行）。超额不阻断提交，但会以 `warn` 返回给你，`is_fully_matched=false` 并等待人工复核（v0.4 当前实现，后续升级为完整三单匹配）。
...
3. **我误点了「标记已付款 Mark Paid」，怎么撤销？**
   当前 v0.4 版本**没有自助回滚**通道。请联系管理员在后台协助修订，并在付款记录的**备注 Notes** 里说明原因，保持审计轨迹。为避免误操作，建议在点按钮前用付款流水号和网银回执做最后一次核对。
...
4. **巡视全局 PO 与合同**
   ...
   - 切到**合同 Contracts**：审核合同条款、到期提醒。合同详情页有**审核 Review** / **补充条款 Amend** 等操作（v0.4 合同模块以管理为主，详细合同流在 v0.5 扩展）。

5. **异常复核**
   - 在**采购申请 Purchase Requisitions** 或**采购订单 Purchase Orders** 列表中，可以用筛选找"单价异常"的记录（当 LLM 价格异常预警落地后，异常单会有标记徽章）。
...
1. **采购经理和部门负责人能同时审批同一张 PR 吗？**
   不能。当前 v0.4 是**单级审批**，一张 PR 只会路由给其中一个角色：金额 < ¥100,000 → 部门负责人；≥ ¥100,000 → 采购经理。多级串签（例如先部门后采购）计划在 v0.5+ 落地。

2. **我能主动把一张路由错了的 PR 拉到自己名下吗？**
   不能直接拉。若某 PR 金额接近阈值但判断应由你审批，请让申请人调整明细后重新提交，或让管理员代为调整路由。

3. **合同模块什么时候完善？**
   v0.4 的合同目前以"记录 + 基本审核"为主；完整合同版本管理、电子签、续签提醒在 v0.5+ 迭代。当前你看到的字段已足够登记一份主合同。
...
五个业务角色的菜单你都能看到，再加上（规划中）管理员控制台：

- **仪表盘 Dashboard**
- **待办审批 My Approvals**（作为审批人的兜底候选）
- **采购申请 / 采购订单 / 合同 / 交货批次 / 付款记录 / 发票** — 全量可见
- **管理员控制台 Admin Console** — *规划中 v0.5+*

### 典型一天

1. **登录后扫一眼异常**
   - 仪表盘顶栏通常安静。只有在下述场景出现任务时才介入。

2. **场景 A：代审批（兜底）**
   - 某部门负责人休假，团队反映审批堵住了。
   - 你进入**待办审批 My Approvals**，看到堆积的审批任务（代码层面：`admin` 可以处置任一 `pending` 任务，见 [`backend/app/services/approval.py:117-119`](../../backend/app/services/approval.py)）。
   - 逐条审阅、做出决定，在**审批意见 Comment** 里注明"代 XXX 审批"以便审计回溯。

3. **场景 B：数据纠错**
   - IT 采购员反馈某张已批准的 PR 明细写错了。你可以在 PO 详情页做必要的调整（目前 v0.4 以取消重做为主要纠错方式，避免绕过审批链）。对误点**标记已付款 Mark Paid** 等场景，也由你协助后台修订并留痕。

4. **场景 C：查看审计日志**
   - 通过直接数据库访问或（规划中）管理员控制台，查看 `audit_logs` 表了解谁在何时做了什么操作。
...
> - **所有代操作必须留痕** — 代审批、代纠错时务必在意见 / 备注里写明原因，方便后续审计追责。

> **管理员控制台（LLM 配置、系统参数、用户管理、审批阈值、公司与部门维护）规划中 v0.5+。**
>
> 当前 v0.4 版本：用户、公司、部门、AI 模型等配置以**后台 SQL / 脚本 / .env 环境变量**方式维护，尚无前端界面。建议管理员同时持有数据库访问权限，或通过 `deploy/scripts/` 下的运维脚本操作。完整的管理员控制台 UI 在 v0.5+ 版本中落地。
...
flowchart TD
    ...
    CHK -- 新用户入职 --> A3[创建用户 / 分配角色<br/>v0.4 通过 SQL/脚本]
    CHK -- 调整阈值 --> A4[修改审批金额阈值<br/>v0.5+ 前端, 当前通过配置]
    CHK -- 无事 --> IDLE((保持只读观察))
    A1 --> LOG[审计日志留痕]
    A2 --> LOG
    A3 --> LOG
...
2. **如何新增一个用户？**
   v0.4 版本：通过后端 CLI 脚本或直接写入 `users` 表（需哈希 password）。v0.5+ 将在管理员控制台提供"新增用户 Add User"表单。在此之前，请参阅 `deploy/scripts/` 或联系后端维护人。
