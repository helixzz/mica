...
1. **登录 Mica**
   - 在 `http://mica.<公司域名>` 输入账号密码，或点击 **通过 SSO 登录 Sign in with SSO** 走企业单点登录。
...
实际采购中，原始 PR 上的需求和最终交付的物料常常**不完全一致**——可能因为供货紧张、型号停产或预算调整。系统现在提供了完整的偏离追踪能力：

**4 种履约类型**：

| 类型 | 含义 | 示例 |
|------|------|------|
| `equivalent`（完全等价） | PO 行与 PR 行 1:1 对应 | 申请 64 台服务器 X，到货 64 台服务器 X |
| `downgraded`（降配） | 同型号但配置缩水，仍计入履约 | 申请 64 台完整服务器，实到 32 台缺 A 配件的服务器 |
| `substitute`（替换型号） | 不同型号顶替原需求 | X 型号缺货，改用 Y 型号 |
| `supplementary`（派生补充） | PO 增加新行，不计入主履约数量 | 为缺 A 配件的服务器单独采购 A 配件 |

**分批转 PO**：当 PR 有多个供应商或部分行尚未确定供应商时，可以先选其中几行转 PO，剩余的之后再转。PR 状态会自动变为 `partially_converted`。

**偏离审批**：单个 `downgraded` 或 `substitute` link 的金额（贡献数量 × PO 单价）超过 `fulfillment.deviation_approval_threshold`（默认 10 万）时，系统自动创建审批单交给采购经理。**审批不阻塞业务**——即使审批未通过，PO 仍会执行；审批结果用于事后追溯和合规记录。

**Dashboard 偏离率卡片**：仪表盘显示过去 30 天的偏离率（降配 + 替换 link 数 / 总履约 link 数），帮助你快速识别供应链问题集中的时段。
...
1. **PR 保存草稿后还能删除吗？**
   可以。草稿（draft）状态的 PR 在详情页可直接**删除 Delete**。审批通过后，如果尚未生成任何采购订单，申请人本人、管理员、采购经理或 IT 采购员可点**作废 Cancel PR**，把 PR 转为 `cancelled`；作废后仍可删除。已生成采购订单的 PR 必须从订单层处理，不能直接作废或删除。
...
3. **PR 批准后发现明细填错了怎么办？**
   一旦 PR 进入 `approved` 状态，你无法再修改明细。若尚未生成采购订单，可点**作废 Cancel PR**（进入 `cancelled`），再复制或新建一张 PR。如果已经**生成采购订单 Convert to PO**，作废按钮不会显示，需要联系采购经理或管理员从订单层处理。养成习惯：提交前复核一次明细。
