...
| `it_buyer` | IT 采购员 | 发起采购申请、跟踪订单进度、登记到货收货；可见全量业务数据 |
| `dept_manager` | 部门负责人 | 审批本部门提交的 PR（小额通道）；仅可见本部门相关数据 |
| `finance_auditor` | 财务审核 | 登记付款、登记发票；可见全量业务数据（财务视图） |
| `procurement_mgr` | 采购经理 | 大额 PR 审批；采购全局视图，统一看板 |
| `requester` | 需求方 | 提交本人 PR；仅可见自己提交的 PR 及其下游业务记录 |
...
- 仅可提交本人的采购需求
- 仅可查看自己提交过的 PR 及其下游订单、合同、付款、到货记录
- 看不到他人的 PR，看不到询价（RFQ）和发票模块

## 2. 权限矩阵

下表用"✓ / ✗ / 条件"描述六角色对核心动作的可用性。"条件"指该动作存在但有附加限制（金额、所属公司、所属部门等）。

| 动作 | admin | it_buyer | dept_manager | finance_auditor | procurement_mgr | requester |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 创建 PR | ✓ | ✓ | ✗ | ✗ | ✓ | ✓ |
| 提交 PR（发起审批） | ✓ | ✓ | ✗ | ✗ | ✓ | ✓ |
...
| 确认付款 | ✓ | ✗ | ✗ | ✓ | ✗ | ✗ |
| 登记发票 | ✓ | ✗ | ✗ | ✓ | ✗ | ✗ |
| 查看 PR | 全部 | 全部 | 本部门 | 全部（只读） | 全部 | 仅自己 |
| 查看 PO / 合同 / 到货 / 付款 | 全部 | 全部 | 本部门 PR 关联 | 全部（只读） | 全部 | 仅自己 PR 关联 |
| 查看 RFQ（询价） | ✓ | ✓ | ✗ | ✓ | ✓ | ✗ |
| 查看发票 | ✓ | ✓ | ✗ | ✓ | ✓ | ✗ |
| 配置系统（公司 / 部门 / 用户 / 阈值 / AI） | ✓ | ✗ | ✗ | ✗ | ✗ | ✗ |
...
自 v1.21.0 起，行级权限严格按下列规则收敛（详见 [decision 0020](https://github.com/helixzz/mica)）：

- **admin / it_buyer / finance_auditor / procurement_mgr** → 看全部业务数据（无行级过滤）
- **dept_manager** → 仅看 `pr.department_id == 我的部门` 的 PR，及其下游 PO / 合同 / 到货 / 付款 / 发票 / 交货计划
- **requester** → 仅看 `pr.requester_id == 我` 的 PR，及其下游记录
- **RFQ 模块**对 `dept_manager` 和 `requester` 完全隐藏

下游记录（PO / Contract / Shipment 等）通过 `entity.po_id → PO.pr_id → 可见 PR` 链路推导可见性。
