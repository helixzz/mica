...
FIELD_PERMISSIONS: dict[str, dict[str, set[str]]] = {
    "purchase_requisition": {
        ...
        "finance_auditor": {
            ...
            "decided_by_id", "decision_comment", "created_at", "updated_at",
            "items",
        },
    },
    "purchase_order": {
        ...
        "procurement_mgr": {"*"},
        "it_buyer": {
            "id", "po_number", "pr_id", "supplier_id", "company_id",
            "status", "currency", "total_amount", "qty_received",
            "source_type", "source_ref", "created_by_id",
    ...
    "invoice": {
        ...
        "dept_manager": {
            "id", "internal_number", "invoice_number", "po_id",
            "invoice_date", "total_amount", "currency", "status",
        },
    },
}
...
def build_field_manifest(resource: str, role: str, all_fields: list[str]) -> FieldManifest:
    ...
    if "*" in perms or not perms:
        vis = {f: True for f in all_fields}
    else:
